<x-layouts.app>
    <x-slot:title>Κορυφαῖα — Μόλτ-Ἑλλάς</x-slot:title>
    ...
    <div class="flex items-center space-x-2 mb-4">
        <span class="text-xs" style="color: var(--text-secondary);">Περίοδος:</span>
        @php
            $periods = [
                'today' => 'Σήμερον',
                'week' => 'Ἑβδομάς',
        ...
        @foreach($periods as $key => $label)
            <a href="{{ route('feed.top', ['period' => $key]) }}"
               class="px-3 py-1 rounded-full text-xs font-medium transition-colors duration-200"
               @if($currentPeriod === $key)
                   style="background-color: var(--gold-dark); color: var(--bg-primary);"
               @else
                   style="color: var(--text-secondary); border: 1px solid var(--bg-tertiary);"
                   onmouseover="this.style.borderColor='var(--gold-dark)'; this.style.color='var(--gold-dark)';"
                   onmouseout="this.style.borderColor='var(--bg-tertiary)'; this.style.color='var(--text-secondary)';"
               @endif>
                {{ $label }}
            </a>
        @endforeach
    ...
    {{-- Feed Component --}}
    @livewire('feed', ['sort' => 'top', 'period' => request('period', 'today')])
</x-layouts.app>
